		...
		$(':input, select,button,a', this).each(function(){
			...
			if(name!=null){
				if (name.indexOf("#index#") >= 0){
                    ...
                    //新增入住情况 第一个顾客下拉框自动赋值
                    if(tableId == "add_customerRoomDes_table" && i == 0){
                        if(name.indexOf("customerId") >= 0){
                            $this.val($("#customerId").val());
                        }
                        if(name.indexOf("customerName") >= 0){
                            $this.val($("#customerName").val());
						}
                        if(name.indexOf("relationship") >= 0){
                            $this.val("15"); //关系是自己
                        }
                        if(name.indexOf("beginDate") >= 0){
                            $this.val($("#inDate").val());
                        }
                    }
				}else{
...
function onStayinTypeChange(item) {
    var value = item.value;
    $("select[name='stayinType']").val(value);
}
